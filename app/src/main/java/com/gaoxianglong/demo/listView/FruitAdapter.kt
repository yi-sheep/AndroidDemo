...
class FruitAdapter(activity: Activity, val resourceId: Int, data: List<Fruit>) :
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (convertView == null) {
            ...
            val myName: TextView = view.findViewById(R.id.name_tv) // 找到text控件
            val myNumber: TextView = view.findViewById(R.id.number_tv) // 找到text控件
            viewHolder = ViewHolder(fruitImage,fruitName,myName,myNumber)
            view.tag = viewHolder
        }else{
            view=convertView
        ...
        if (item != null) {
            ...
            view.name_tv.text = item.my_name
            view.number_tv.text = item.my_number
        }
        return view
    }
    // inner class 是用来定义内部类的
    inner class ViewHolder(val fruitImage:ImageView,val fruitName:TextView,val myName:TextView,val myNumber:TextView)
}